...
KeyboardDriver::KeyboardDriver(InterruptManager* manager, KeyboardEventHandler *handler, TaskManager* taskManager)
...
//reads port and sets last key pressed as pressed key
uint32_t KeyboardDriver::HandleInterrupt(uint32_t esp) {
    uint8_t key = dataport.Read();
