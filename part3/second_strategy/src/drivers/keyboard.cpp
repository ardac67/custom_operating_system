...
KeyboardDriver::KeyboardDriver(InterruptManager* manager, KeyboardEventHandler *handler, TaskManager* taskManager,GlobalDescriptorTable* gdt)
    ...
    if(key < 0x80)
    {
        switch(key)
        {
            ...
            default:
            {
                //printf("KEYBOARD 0x");
                //printfHex(key);
                //this->taskManager->BlockedToReady();
                break;
            }
        }
    }
...
uint32_t KeyboardDriver::GetLastKey(){
...
uint32_t KeyboardDriver::HandleInterrupt(uint32_t esp) {
    ...
    if(key < 0x80) {
        //if any key pressed high prior program added to task list
        Task * high_task = new Task(gdt, high_prior_program, taskManager,HIGH);
        taskManager->AddTask(high_task);
    }
    return esp;
}
uint32_t KeyboardDriver::GetLastKey(){
...
void KeyboardDriver::ClearLastKey(){
    last_key_pressed = 0;
}
